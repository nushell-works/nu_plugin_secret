...
use nu_plugin_secret::{SecretBinary, SecretBool, SecretFloat, SecretInt, SecretString};
...
mod side_channel_tests {
    ...
    fn test_integer_comparison_timing_attacks() {
        ...
        for &test_value in &test_values {
            ...
            let avg_nanos = case_timings
                .iter()
                .map(|d| d.as_nanos() as u64)
                .sum::<u64>()
                / case_timings.len() as u64;
        ...
        if time_ratio > 2.0 {
            println!(
                "Warning: Integer comparison timing variance detected (ratio: {:.2})",
                time_ratio
            );
        }
    }
    ...
    #[test]
    #[ignore] // Temporarily disabled due to system timing variance
    fn test_binary_data_sidechannel_resistance() {
        ...
        // Allow reasonable variance for system timing noise and micro-optimizations
        // Note: Perfect constant-time is difficult to achieve due to CPU optimizations,
        // cache effects, and system variance, but we should still detect major timing leaks
        assert!(
            time_ratio < 4.0,
    ...
    fn test_cache_timing_resistance() {
        ...
        if time_ratio > 3.0 {
            println!(
                "Warning: Cache timing variance detected (ratio: {:.2})",
                time_ratio
            );
        }
    }
    ...
    fn test_branch_prediction_resistance() {
        ...
        if time_ratio > 5.0 {
            println!(
                "Warning: Large branch timing difference (ratio: {:.2})",
                time_ratio
            );
        }
    }
    ...
    fn test_power_analysis_simulation() {
        ...
        if variance_ratio > 2.0 {
            println!(
                "Warning: Potential power analysis vulnerability (ratio: {:.2})",
                variance_ratio
            );
        }
    }
    ...
    fn test_electromagnetic_emanation_simulation() {
        ...
        if consistency_ratio > 2.5 {
            println!(
                "Warning: Potential EM emanation vulnerability (ratio: {:.2})",
                consistency_ratio
            );
        }
    }
...
mod statistical_analysis {
    ...
    fn test_timing_statistical_significance() {
        ...
        if coefficient_of_variation > 0.5 {
            println!(
                "Warning: High timing variability detected (CV: {:.4})",
                coefficient_of_variation
            );
        }
    }
    ...
    fn test_timing_distribution_normality() {
        ...
        let skewness = sample_times
            .iter()
            .map(|&x| ((x - mean) / std_dev).powi(3))
            .sum::<f64>()
            / sample_times.len() as f64;

        let kurtosis = sample_times
            .iter()
        ...
        if skewness.abs() > 2.0 {
            println!(
                "Warning: High skewness detected ({:.4}) - timing distribution is asymmetric",
                skewness
            );
        }
...
mod security_performance_tests {
    ...
    fn test_constant_time_operation_performance() {
        ...
        for size in test_sizes {
            ...
            println!(
                "Constant-time comparison for {} chars: {}ns",
                size,
                duration.as_nanos()
            );
    ...
    fn test_adversarial_performance() {
        ...
        for (i, input) in adversarial_inputs.iter().enumerate() {
            ...
            println!(
                "Adversarial input {} performance: {}ns",
                i,
                duration.as_nanos()
            );
