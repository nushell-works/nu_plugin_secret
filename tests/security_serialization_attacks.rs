        ...
        match json_result {
            Ok(json) => {
                assert!(
                    ...
                    "Secret content exposed in JSON: {}",
                    json
                );
                assert!(
                    json.contains("redacted") || json.contains("SecretString"),
                    "JSON should contain redacted indicator: {}",
                    json
                );
            }
            Err(_) => {
    ...
    fn test_bincode_serialization_protection() {
        ...
        match bincode_result {
            Ok(bytes) => {
                ...
                assert!(
                    !bytes_str.contains("bincode_secret_test"),
        ...
        match yaml_result {
            Ok(yaml) => {
                assert!(
                    ...
                    "Secret content exposed in YAML: {}",
                    yaml
                );
            }
            ...
            match json_result {
                Ok(json) => {
                    assert!(
                        ...
                        "Secret type {} exposed sensitive data in JSON: {}",
                        i,
                        json
                    );
                }
    ...
    fn test_serialization_bomb_resistance() {
        ...
        match json_result {
            Ok(json) => {
                ...
                assert!(
                    json.len() < 100_000,
                    "Serialized output suspiciously large: {} bytes",
                    json.len()
                );
                ...
                assert!(
                    ...
                    "Large record content exposed in serialization"
                );
            }
...
mod nushell_integration_tests {
    ...
    fn test_nushell_value_serialization_safety() {
        ...
        match json_result {
            Ok(json) => {
                assert!(
                    !json.contains("nushell_secret"),
                    "Secret exposed through Nushell Value serialization: {}",
                    json
                );
            }
    ...
    fn test_plugin_communication_serialization() {
        ...
        match bincode_result {
            Ok(bytes) => {
                let bytes_str = String::from_utf8_lossy(&bytes);
                assert!(
                    !bytes_str.contains("plugin_secret"),
