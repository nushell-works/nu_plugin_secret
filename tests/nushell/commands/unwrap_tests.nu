...
export def test_unwrap_all_types [] {
    # String
    let str_secret = "test-string" | secret wrap
    ...
    # Float
    let float_secret = 3.14159 | secret wrap
    assert_eq ($float_secret | secret unwrap) 3.14159 "Float unwrap failed"
    
    # Record
    let record_secret = {key: "value", num: 123} | secret wrap
    ...
    # List
    let list_secret = ["item1", "item2", "item3"] | secret wrap
...
export def test_unwrap_data_integrity [] {
    ...
    let secret = $complex_record | secret wrap
    let revealed = $secret | secret unwrap
...
export def test_unwrap_string_variants [] {
    ...
    for test_str in $test_strings {
        let secret = $test_str | secret wrap
        let revealed = $secret | secret unwrap
...
export def test_unwrap_round_trip_consistency [] {
    ...
    for test_int in $test_data.test_integers {
        let secret = $test_int | secret wrap
        let revealed = $secret | secret unwrap
    ...
    for test_float in $test_data.test_floats {
        let secret = $test_float | secret wrap
        let revealed = $secret | secret unwrap
    ...
    for test_bool in $test_data.test_booleans {
        let secret = $test_bool | secret wrap
        let revealed = $secret | secret unwrap
...
export def test_unwrap_multiple_calls [] {
    let secret = "consistent-test" | secret wrap
...
export def test_unwrap_performance [] {
    # Create many secrets
    let secrets = 0..100 | each { |i| $"secret-($i)" | secret wrap }
...
export def test_unwrap_edge_cases [] {
    # Very long string
    let long_secret = (get_long_test_string 10000) | secret wrap
    ...
    # Extreme numbers
    let max_int = 9223372036854775807 | secret wrap
    assert_eq ($max_int | secret unwrap) 9223372036854775807 "Max int should unwrap correctly"
    
    let min_int = -9223372036854775808 | secret wrap
