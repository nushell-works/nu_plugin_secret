...
use nu_plugin_secret::{SecretString, SecretInt, SecretBinary, SecretRecord, SecretList};
...
mod memory_safety_tests {
    ...
    fn test_secret_record_memory_zeroing() {
        ...
        record.insert("api_key", nu_protocol::Value::string("secret123", nu_protocol::Span::test_data()));
        record.insert("password", nu_protocol::Value::string("password456", nu_protocol::Span::test_data()));
    ...
    fn force_memory_pressure() {
        ...
        let _pressure: Vec<Vec<u8>> = (0..100)
            .map(|_| vec![0u8; 1024])
            .collect();
    }
    ...
    fn stack_contains_pattern(stack_data: &[u8], pattern: &[u8]) -> bool {
        stack_data.windows(pattern.len()).any(|window| window == pattern)
    }
}
...
mod additional_memory_tests {
    use nu_plugin_secret::{SecretString, SecretInt};
    ...
    fn test_common_string_cleanup() {
        ...
        for test_case in &test_cases {
            ...
            assert!(
                ...
                "Common string '{}' found in heap after drop", test_case
            );
        }
    }
...
mod memory_safety_benchmarks {
    ...
    fn benchmark_secret_string_performance() {
        ...
        println!("SecretString performance: {}ns per operation", per_operation);
    }
    ...
    fn benchmark_memory_overhead() {
        ...
        println!("String: {} bytes, SecretString: {} bytes, Overhead: {} bytes", 
