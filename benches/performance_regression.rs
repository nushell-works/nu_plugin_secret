        ...
        group.bench_with_input(
            ...
            |b, s| {
                b.iter_custom(|iters| {
                    ...
                    performance_monitoring::get_monitor().record(
                        performance_monitoring::Measurement {
...
fn bench_binary_optimization_regression(c: &mut Criterion) {
    ...
    for (pattern_name, data) in test_patterns {
        ...
        group.bench_with_input(
            BenchmarkId::new("creation", pattern_name),
            &data,
            |b, d| {
                b.iter_custom(|iters| {
                    let start = Instant::now();
...
fn bench_memory_usage_regression(c: &mut Criterion) {
    ...
    for size in sizes {
        group.bench_with_input(
            BenchmarkId::new("bulk_creation", size),
            &size,
            |b, &s| {
                b.iter_custom(|iters| {
                    let start = Instant::now();
        ...
        b.iter_custom(|iters| {
            ...
            if avg_ms > targets.startup_time_ms as u128 / 1000 {
                eprintln!(
                    ...
                    avg_ms, targets.startup_time_ms / 1000
                );
            }
