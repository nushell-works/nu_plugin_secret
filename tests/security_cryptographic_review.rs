...
use nu_plugin_secret::{SecretString, SecretInt, SecretBinary};
...
mod cryptographic_security_tests {
    ...
    fn test_constant_time_string_comparison() {
        ...
        for (str1, str2, expected) in test_cases {
            ...
            assert_eq!(result, expected, "Comparison failed for '{}' vs '{}'", str1, str2);
        }
        ...
        if time_ratio > 2.0 {
            println!("Warning: String comparison may not be constant-time (ratio: {:.2})", time_ratio);
        }
    }
    ...
    fn test_constant_time_integer_comparison() {
        ...
        if time_ratio > 2.0 {
            println!("Warning: Integer comparison timing variance (ratio: {:.2})", time_ratio);
        }
    }
    ...
    fn test_constant_time_binary_comparison() {
        ...
        if time_ratio > 10.0 {
            println!("Warning: Significant timing variance in binary comparison (ratio: {:.2})", time_ratio);
    ...
    fn test_cryptographic_randomness_handling() {
        ...
        let random_secrets: Vec<SecretInt> = test_values.iter()
        ...
        let random_bytes: Vec<u8> = test_values.iter()
        ...
        let binary_secrets: Vec<SecretBinary> = chunks.iter()
        ...
        println!("Random data handling test completed - {} int secrets, {} binary secrets", 
                random_secrets.len(), binary_secrets.len());
    }
    ...
    fn test_secure_comparison_properties() {
        ...
        assert!((secret1 == secret2) == (secret2 == secret1), "Symmetry test failed");
        ...
        assert!(different1 != different2, "Different values incorrectly compared as equal");
...
mod cryptographic_best_practices {
    ...
    fn test_cryptographic_requirements() {
        ...
        assert_eq!(result1, result2, "Deterministic behavior requirement failed");
...
mod cryptographic_performance {
    ...
    fn test_cryptographic_operation_performance() {
        ...
        for size in test_sizes {
            ...
            println!("{} byte key comparison: {}ns per operation", size, per_op_nanos);
            ...
            assert!(per_op_nanos < max_allowed_nanos, 
                   "Cryptographic operation too slow for {} bytes: {}ns (max: {}ns)", 
                   size, per_op_nanos, max_allowed_nanos);
        }
    }
    ...
    fn test_cryptographic_scalability() {
        ...
        assert!(perf_ratio < 10.0,
                "Poor scalability detected (ratio: {:.2})", perf_ratio);
    }
}