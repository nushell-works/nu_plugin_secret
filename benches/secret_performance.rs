...
fn bench_secret_string(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        // Benchmark creation
        group.bench_with_input(BenchmarkId::new("creation", size), &test_string, |b, s| {
            b.iter(|| SecretString::new(s.clone()))
        });

        // Benchmark reveal (unwrap)
        let secret = SecretString::new(test_string.clone());
        group.bench_with_input(BenchmarkId::new("reveal", size), &secret, |b, s| {
            b.iter(|| s.reveal())
        });

        // Benchmark clone
        group.bench_with_input(BenchmarkId::new("clone", size), &secret, |b, s| {
            b.iter(|| s.clone())
        });

        // Benchmark display (redacted)
        group.bench_with_input(BenchmarkId::new("display", size), &secret, |b, s| {
            b.iter(|| format!("{}", s))
        });
    }
...
fn bench_secret_int(c: &mut Criterion) {
    ...
    for value in test_values {
        // Benchmark creation
        group.bench_with_input(BenchmarkId::new("creation", value), &value, |b, v| {
            b.iter(|| SecretInt::new(*v))
        });

        // Benchmark reveal
        let secret = SecretInt::new(value);
        group.bench_with_input(BenchmarkId::new("reveal", value), &secret, |b, s| {
            b.iter(|| s.reveal())
        });
    }
...
fn bench_secret_record(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        // Benchmark creation
        group.bench_with_input(BenchmarkId::new("creation", size), &record, |b, r| {
...
fn bench_secret_list(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        // Benchmark creation
        group.bench_with_input(BenchmarkId::new("creation", size), &list, |b, l| {
...
fn bench_secret_binary(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        // Benchmark creation
        group.bench_with_input(BenchmarkId::new("creation", size), &data, |b, d| {
        ...
        group.bench_with_input(BenchmarkId::new("length", size), &secret, |b, s| {
            b.iter(|| s.len())
        });
    }
...
fn bench_secret_float(c: &mut Criterion) {
    ...
    for (i, value) in test_values.iter().enumerate() {
        // Benchmark creation
        group.bench_with_input(BenchmarkId::new("creation", i), value, |b, v| {
            b.iter(|| SecretFloat::new(*v))
        });

        // Benchmark reveal
        let secret = SecretFloat::new(*value);
        group.bench_with_input(BenchmarkId::new("reveal", i), &secret, |b, s| {
            b.iter(|| s.reveal())
        });
    }
...
fn bench_overhead_comparison(c: &mut Criterion) {
    ...
    group.bench_function("regular_string_clone", |b| b.iter(|| test_string.clone()));
    ...
    group.bench_function("secret_string_clone", |b| b.iter(|| secret_string.clone()));
    ...
    group.bench_function("regular_int_clone", |b| b.iter(|| test_int.clone()));

    // Secret int operations
    let secret_int = SecretInt::new(test_int);
    group.bench_function("secret_int_clone", |b| b.iter(|| secret_int.clone()));
